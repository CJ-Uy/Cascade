-- Migration: Fix section progression to work with linked request model
-- Date: 2026-01-06
-- Description: When using linked requests (each section is a separate request),
--              only show the current section's progress, not future sections
...
BEGIN
  ...
  -- Build sections array - ONLY show the current section and completed ones
  -- This prevents showing future sections that belong to separate linked requests
  ...
  FROM (
    ...
    WHERE ws.chain_id = v_workflow_chain_id
      -- CRITICAL: Only show sections up to and including the current one
      AND ws.section_order <= v_current_section_order
  ) sections_query;
...
IS 'Get workflow progress for a request. In linked request model, each request represents ONE section, so only shows current and prior sections.';
