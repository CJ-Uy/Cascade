...
export function FormFiller({ formFields, onSubmit }: FormFillerProps) {
  ...
  const renderField = (field: FormField) => {
    ...
    switch (field.type) {
        ...
        case 'table':
            return <RepeaterFiller field={field} value={formData[field.id] || []} onChange={(value) => handleValueChange(field.id, value)} />;
        case 'file-upload':
            const fileName = formData[field.id] ? formData[field.id].name : 'No file chosen';
            return fieldWrapper(field.label, 
                <div className="flex items-center space-x-2">
                    <Input 
                        {...commonProps} 
                        type="file"
                        onChange={(e) => handleValueChange(field.id, e.target.files ? e.target.files[0] : null)}
                        className="flex-grow"
                    />
                    {formData[field.id] && <span className="text-sm text-muted-foreground">{fileName}</span>}
                </div>
            );
        default:
            return null;
  ...
  return (
    <div className="p-4 border rounded-md bg-white">
        ...
        <Button onClick={() => onSubmit(formData)} className="mt-6 bg-emerald-600 hover:bg-emerald-500">
            Submit Requisition
        </Button>
    </div>
  );
...
function RepeaterFiller({ field, value, onChange }: { field: FormField, value: any[], onChange: (value: any[]) => void }) {
    const addRow = () => {
        onChange([...value, {}]);
    };
        ...
        <div className="mb-6 p-4 border rounded-lg bg-gray-50">
            ...
            <div className="space-y-4">
                {value.map((row, rowIndex) => (
                    <div key={rowIndex} className="p-4 border rounded-md bg-white relative">
                        <div className="space-y-4">
                            {field.columns?.map(col => (
                                <div key={col.id}>
                                    ...
                                    <Input 
                                        ...
                                        value={row[col.id] || ''}
                                        onChange={(e) => handleRowChange(rowIndex, col.id, e.target.value)}
                                    />
                                </div>
                            ))}
                        </div>
                        <Button variant="ghost" size="icon" className="absolute top-1 right-1" onClick={() => removeRow(rowIndex)}>
