...
const generateLabelFromKey = (key: string): string => {
  const result = key.replace(/([A-Z])|_/g, (match, p1) =>
    p1 ? ` ${p1}` : " ",
  );
    ...
    if (sampleItem) {
      ...
      if (!field.optional) {
        ...
        arraySchema = arraySchema.min(
          1,
          `At least one ${singularItemName} is required.`,
        );
      }
      ...
      if (!field.optional) {
        shape[fieldName] = z
          .string()
          .min(1, `${field.title} (list) cannot be empty.`);
      }
    } else {
...
export default function CreateRequisitionPage() {
  ...
  useEffect(() => {
    ...
    if (selectedTemplateKey && templatesData[selectedTemplateKey]) {
      ...
      const { schema, defaultValues } = generateFormConfig(
  ...
  const selectedTemplate =
    templatesData && selectedTemplateKey
      ? templatesData[selectedTemplateKey]
      : null;
              ...
              <Form {...form}>
                ...
                >
                  {selectedTemplate.values.map((fieldConfig) => {
                    ...
                    let sampleItemForInference: Record<string, any> | null =
                      null;
                    ...
                    if (isSpecialArrayDefault(fieldConfig.default)) {
                      ...
                      sampleItemForInference = fieldConfig.default[0] as Record<
                        string,
                        any
                      >;
