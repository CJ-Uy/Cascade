...
export function RequisitionProgressBar({
  currentStep,
  totalSteps,
  status,
}: RequisitionProgressBarProps) {
  ...
  return (
    <div className="flex h-2 w-full gap-0.5 overflow-hidden rounded-full bg-gray-200">
      {stepsArray.map((step, index) => (
        <div
          key={step}
          className={cn("h-full rounded-sm", getSegmentColor(index))}
