-- Fix workflow progress RPC function
-- Issue: section_order is 0-indexed in workflow_sections, but we were comparing against 1-indexed values
...
BEGIN
  ...
  -- For now, we'll use placeholder logic: first section (0), first step (1)
  v_current_section := 0;  -- 0-indexed to match section_order
  v_current_step := 1;     -- 1-indexed to match step_number
  ...
  SELECT jsonb_build_object(
    ...
    'current_section', v_current_section + 1,  -- Return 1-indexed for display
...
COMMENT ON FUNCTION get_document_workflow_progress IS 'Returns complete workflow progress information for a document including all sections and steps. Handles 0-indexed section_order and 1-indexed step_number correctly.';
