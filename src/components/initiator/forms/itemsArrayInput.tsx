...
export function ItemsArrayInput({ control, name, label, optional, form }: ItemsArrayInputProps) {
					...
					{fields.map((item, index) => (
						...
						>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel className="text-xs md:hidden">Quantity</FormLabel>
										<FormControl>
											<Input
												type="number"
												placeholder="1"
							...
							<FormItem>
								...
								>
									<ItemTotalCost itemIndex={index} control={control} arrayName={name} />
								</div>
							</FormItem>
							{/* Remarks */}
							<FormField
