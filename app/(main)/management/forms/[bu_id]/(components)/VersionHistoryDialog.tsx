...
import { restoreFormVersionAction } from "@/app/(main)/management/forms/actions";
...
interface VersionHistoryDialogProps {
  ...
  formName: string;
  formId: string;
  onRestore: () => void; // Callback to trigger re-fetch in FormList
}
...
export function VersionHistoryDialog({
  ...
  useEffect(() => {
    ...
    const fetchVersionHistory = async () => {
      ...
      if (fetchCurrentError || !currentForm) {
        console.error("Could not fetch form details:", fetchCurrentError);
        setLoading(false);
        return;
      }
  ...
  const handleRestoreVersion = async (versionId: string) => {
    ...
    try {
      await restoreFormVersionAction(versionId, pathname);
      toast.success("Form version restored successfully!");
      onClose(); // Close the dialog
      onRestore(); // Trigger re-fetch in FormList
    } catch (error: any) {
      console.error("Failed to restore form version:", error);
