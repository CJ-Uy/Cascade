-- Fix get_request_workflow_progress to not mark steps complete across different sections
-- Currently when Step 1 is approved in Section 0, it marks Step 1 as complete in ALL sections
-- This is because is_completed only counts approvals by number, not by section
...
IS 'Returns complete workflow progress for a request including all sections with their initiator roles, approval steps, and current status. FIXED: Steps are only marked complete within their own section, not across all sections. Used for displaying workflow details in request viewer.';
