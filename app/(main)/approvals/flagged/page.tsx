...
export default function FlaggedRequisitionsPage() {
  const [flaggedRequisitions, setFlaggedRequisitions] = useState<Requisition[]>(dummyFlaggedRequisitions);
  const [isDetailsDialogOpen, setIsDetailsDialogOpen] = useState(false);
  const [selectedRequisition, setSelectedRequisition] = useState<Requisition | null>(null);
  const [isConfirmDialogOpen, setIsConfirmDialogOpen] = useState(false);
  const [actionType, setActionType] = useState<'unflag' | 'resolve' | null>(null);
  ...
  const handleAction = (requisition: Requisition, type: 'unflag' | 'resolve') => {
    setSelectedRequisition(requisition);
    setActionType(type);
  ...
  const handleConfirmAction = () => {
    ...
    setFlaggedRequisitions(prev => prev.filter(req => req.id !== selectedRequisition.id));
    ...
    { key: 'submittedDate', header: 'Submitted' },
    {
      ...
      render: (requisition: Requisition) => (
        <div className="flex justify-end gap-2">
          ...
          <Button variant="secondary" size="sm" onClick={() => handleAction(requisition, 'unflag')}>
            <FlagOff className="mr-2 h-4 w-4" /> Unflag
          </Button>
          <Button variant="default" size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => handleAction(requisition, 'resolve')}>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleConfirmAction} className={
              actionType === 'unflag' ? 'bg-yellow-600 hover:bg-yellow-700' :
