-- RPC function to get initiatable templates for a user in a business unit
-- This replaces the complex query in getInitiatableForms and fixes the approval_workflows join issue
      ...
      'fields', (
        SELECT json_agg(
          json_build_object(
            ...
            'options', (
              SELECT array_agg(fo.value ORDER BY fo.order_index)
...
COMMENT ON FUNCTION get_initiatable_templates IS 'Returns all active templates in a business unit that the current user can initiate, along with their workflow information.';
