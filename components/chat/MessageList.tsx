      ...
      <div className="p-4">
        {messages.map((message, index) => {
          ...
          return (
            ...
            >
              <div className={`flex max-w-[70%] ${isCurrentUserMessage ? 'flex-row-reverse' : 'flex-row'} items-end space-x-2`}>
                ...
                {!showAvatar && !isCurrentUserMessage && <div className="w-8" />}

                {/* Message Content */}
                <div className={`flex flex-col ${isCurrentUserMessage ? 'items-end' : 'items-start'}`}>
                  {/* Sender name and time - Only show for other users */}
                  {showAvatar && !isCurrentUserMessage && (
                    <div className={`flex items-center space-x-2 mb-1 ${isCurrentUserMessage ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  {!showAvatar && showTime && (
                    <span className={`text-xs text-muted-foreground mt-1 ${isCurrentUserMessage ? 'text-right' : 'text-left'}`}>
                      {formatMessageTime(message.createdAt)}
                    </span>
                  )}
