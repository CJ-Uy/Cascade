...
export default function EmployeesPage() {
  ...
  const [noRolePeople, setNoRolePeople] = useState<NoRolePerson[]>(
    dummyNoRolePeopleData,
  );
  ...
  const handleSaveEmployee = (employeeToSave: Employee) => {
    if (employeeToSave.id.startsWith("emp_")) {
      // Existing employee
      setEmployees(
        employees.map((emp) =>
          emp.id === employeeToSave.id ? employeeToSave : emp,
        ),
      );
    } else {
      // New employee
      setEmployees([
        ...employees,
        { ...employeeToSave, id: `emp_${Date.now()}` },
      ]);
    }
    ...
    <div className="p-4 md:p-8">
      ...
      <Tabs defaultValue="current-employees" className="w-full">
        <div className="flex items-center justify-between">
          <TabsList>
            <TabsTrigger value="current-employees">
              Current Employees
            </TabsTrigger>
        ...
        <TabsContent value="no-role-people" className="mt-4">
          <NoRolePeopleTable people={noRolePeople} onAddToBu={handleAddToBu} />
        </TabsContent>
      </Tabs>
