-- Migration: Add RPC function for BU Admins to assign roles within their business unit
-- Date: 2026-01-06

-- Function to allow BU Admins to update employee roles within their business unit
...
BEGIN
  ...
  -- Verify employee is in the same organization (for Org/BU Admins)
  -- Check via the business unit's organization
...
GRANT EXECUTE ON FUNCTION update_employee_roles_in_bu(UUID, UUID, TEXT[]) TO authenticated;

-- Add comment
COMMENT ON FUNCTION update_employee_roles_in_bu IS 'Allows BU Admins to update employee roles within their business unit. Also usable by Organization Admins and Super Admins.';
