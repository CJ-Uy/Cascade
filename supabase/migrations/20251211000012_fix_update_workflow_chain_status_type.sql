-- Fix update_workflow_chain_status to use correct enum type
-- The workflow_chains table uses approval_workflow_status, not workflow_chain_status

DROP FUNCTION IF EXISTS update_workflow_chain_status(UUID, TEXT);
...
COMMENT ON FUNCTION update_workflow_chain_status IS 'Updates the status of a workflow chain (draft, active, archived). Uses approval_workflow_status enum type.';
